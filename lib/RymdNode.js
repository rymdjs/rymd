(function() {
  ...
  var onresource  = function(peerName, data, connection) {
    ...
    logger.debug('New Resource data', data.data.data, data.data.guid);

    this.trigger('resource', peerName, data, connection);
  ...
  RymdNode.prototype = function() {
    ...
    var connect = function(identity) {
      ...
      this.verifier.lookupIdentity(identity)
      .then(function(peer) {
        //TODO: Verify identity: Send challenge to other peer, let them sign challenge, verify with pubkey

        var endpointKeys = Object.keys(peer.endpoints);

        if (endpointKeys.length) {

          console.log("peer.endpoints before: ");
          console.log(peer.endpoints);

          var lastEndpoint = peer.endpoints[endpointKeys.pop()];

          console.log("peer.endpoints after: ");
          console.log(peer.endpoints);

          for(var i=0; i<endpointKeys.length; ++i) {

            var endpoint = peer.endpoints[endpointKeys[i]];
          ...
          this.network.connect(identity, lastEndpoint)
  ...
  */
          ...
          logger.global('No connected endpoints');
        }
