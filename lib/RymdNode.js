(function() {
  ...
  function RymdNode(options) {
    ...
    this.network.on('request', function(peerName, data, connection) {
      ...
      this.store.getResourceMetadata(data.guid)
        .then(this.store.loadResourceData.bind(this.store))
        .then(connection.sendResource.bind(connection));

    }.bind(this));

    this.network.on('resource', function(peerName, resource, connection) {
