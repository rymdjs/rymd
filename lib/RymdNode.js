    ...
    var onAuthChallenge = function(peerName, data, connection) {
      ...
      .then(function(data) {
        ...
        var dataView = new Uint8Array(data),
            theirNonce = dataView.subarray(0, 16),
            idView = dataView.subarray(16),
        ...
        Utils.stringToArrayBuffer(peerName).then(function(peerNameBuf) {
          ...
          this.verifier.lookupIdentity(peerName).then(function(peer) {
            var onAuthLastResponse = function(peerName, data) {
              this.crypto.decryptData(privKey, new Uint8Array(data.data)).then(function(responseMyNonceBuf) {
                var responseMyNonce = new Uint8Array(responseMyNonceBuf);
                for(var i = 0; i < responseMyNonceBuf.byteLength; i++) {
