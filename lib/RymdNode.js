(function() {
  ...
  var onresource  = function(peerName, data) {
    ...
    this.store.getResourceMetadata(data.data.guid).then(function(resource) {
  ...
  function RymdNode(options) {
    ...
    this.network.on('data', function() { logger.global('WHEN DOES ONDATA GET CALLED? DATA DRAGON DATA DRAGON'); });
    logger.separator();
  }
  ...
  RymdNode.prototype = function() {
    ...
    var connect = function(identity) {
      ...
      this.verifier.lookupIdentity(identity).then(function(peer) {
        ...
        this.network.connect(identity, peer.endpoints[Object.keys(peer.endpoints).pop()]).then(function(connection) {
          ...
          peer.connection.on('data', function() { logger.global('WHEN DOES ONDATA GET CALLED? DATA DRAGON DATA DRAGON'); });
    ...
    var requestResource = function(guid) {
      return this.store.getResourceMetadata(guid)
