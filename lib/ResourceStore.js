(function() {
	...
	ResourceStore.prototype = function() {
      ...
      var _decryptResource = function(encMetadata, masterKey) {
        return this._crypto.decryptData(masterKey, new Uint8Array(encMetadata)).then(Utils.arrayBufferToBinaryString).then(function(str) {
          ...
          return dataStore().all().then(function(items) {
            var promises = items.map(function(item) {
              if(/-meta$/.test(item.guid)) {
                return _decryptResource.call(item.data, masterKey);
              }
