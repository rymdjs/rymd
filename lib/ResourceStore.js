(function() {
  ...
  ResourceStore.prototype = function() {
    ...
    var createResource = function(name, data, author, type) {
      ...
      var id = Rymd.Utils.guid();

      return Q.all([
        crypto.generateSymmetricKey(),
        crypto.hashBlob(data)
      ]).spread(function(_key, dataHash) {
        key = _key;
        return [
          crypto.encryptBlob(key, data),
          crypto.hashString(name+author+1+type+id+timestamp+dataHash)
        ];
      }).spread(function(encData, hash) {
