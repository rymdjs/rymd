(function() {
    ...
    function ResourceStore(dataStore, crypto, stores) {
    ...
    var getMasterKey = function() {
      ...
      var key;
      var keyStore = this._store.use(this._stores.keyStore);
	...
	ResourceStore.prototype = function() {
      var createResource = function(name, data, author, type) {
        ...
        this._crypto.generateSymmetricKey().then(function(_key) {
          ...
          resource = new Resource(name, encData, author, 1, type);
          return crypto.exportKey(key);
        }).then(function(rawKey) {
      ...
      var saveResource = function(resource) {
        var crypto = this._crypto;
        var store = this._store.use(this._stores.dataStore);
      ...
      var getResource = function(guid) {
        return this._store.use(this._stores.dataStore).get(guid);
      };
      ...
      var getResources = function() {
        ...
        getMasterKey.call(this).then(function(masterKey) {
          store.all().then(function(items) {
            items.forEach(function(item) {
              if(/-meta/.test(item.guid)) {
                crypto.decryptData(masterKey, new Uint8Array(item.data)).then(function(data) {
                  Utils.arrayBufferToBinaryString(data).then(function(metadata) {
