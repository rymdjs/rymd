  ...
  ResourceStore.prototype = function() {
    ...
    var setPrivateKey = function(key, identity) {
      ...
      if(typeof key === 'string') {
        return crypto.importKey('private', 'encrypt', Utils.base64ToUint8Array(key))
        .then(crypto.exportKey)
        .then(function(key) {
            return keyStore().save(identity+'-rsa-priv', key);
        })
      } else {
        return keyStore().save(identity+'-rsa-priv', key);
      }
    ...
    var getPrivateKey = function(identity, purpose) {
      ...
      var crypto = this._crypto;
      var keyStore = this._keyStore;
      return keyStore().get(identity+'-rsa-priv').then(
        function(rawKey){
          if(!rawKey) {
            return null;
          }
          return crypto.importKey('private', purpose || 'encrypt', new Uint8Array(rawKey.data));
       });
    };

    var getResourceHash = function(metadata, data) {
      var crypto = this._crypto;
      return crypto.hashData(new Uint8Array(data)).then(function(dataHash) {
        return crypto.hashString(metadata.name+metadata.author+metadata.version+metadata.type+metadata.id+metadata.timestamp+dataHash);
      });
    };
    ...
    var createResource = function(name, data, author, type) {
      ...
      }.bind(this)).then(function(_encData) {
        encData = _encData;
        return this.getResourceHash(metadata, encData)
      }.bind(this))
      .then(function(hash) {
        metadata.hash = hash;
        resource = new Resource(metadata, encData);
        return crypto.exportKey(key);
      }).then(function(rawKey) {
        return keyStore().save(resource.id+'-k'+metadata.version, rawKey);
      }).then(function() {
        return resource;
      });
    };
    ...
    var saveResource = function(resource) {
      ...
      return this.getResourceHash(resource.metadata, resource.data).then(function(hash) {
        if(typeof resource.data !== 'undefined' && hash !== resource.metadata.hash) {
          throw new Error('Resource hash did not match!');
        }
      }.bind(this))
      .then(getMasterKey.bind(this))
    ...
    return {
      ...
      getResourceHash: getResourceHash
    };
  }();
